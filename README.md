...
**Fase 1 (análisis y arquitectura)** se hizo en una conversación aparte
con Claude (claude.ai), incluyendo la lectura completa del PDF del reto
y del documento de sustentación, para producir
`docs/00-arquitectura-y-decisiones.md`.

**Fases 2 a 5 (diseño detallado, implementación de backend y frontend,
consultas SQL, datos de semilla, y este README)** se hicieron con
Claude Code, en una sesión interactiva donde Claude:

- Escribió la totalidad del código de `backend/` y `frontend/` (schema
  de Prisma, cada módulo/controller/service/DTO, todas las vistas de
  React, el script de seed, y `queries.sql`).
- Tomó decisiones técnicas no cubiertas por el diseño (versiones de
  dependencias, CommonJS vs. ESM, manejo del refresh token como token
  opaco en vez de JWT firmado) — documentadas y justificadas en los
  archivos `README-fase*-notas.md` en vez de aplicadas en silencio.
- Instaló y configuró PostgreSQL de forma nativa en Windows cuando
  Docker Desktop no pudo iniciar en la máquina de desarrollo (falta de
  soporte de virtualización).
- Probó el backend en caliente contra una base de datos real (`curl`)
  y el frontend en un navegador real (Chrome, vía la extensión de
  Claude), no solo compilación/build — y en ese proceso encontró y
  corrigió **dos bugs genuinos** antes de que llegaran a considerarse
  terminados: un bug de rotación de refresh tokens en el backend (el
  `id` embebido en la cookie nunca coincidía con el de la fila en la
  base de datos) y un bug de UX en el frontend (la sesión no
  sobrevivía a un refresh de página).
- Redactó las 8 consultas de `queries.sql` a partir del enunciado
  exacto que el usuario proporcionó (extraído del PDF original del
  reto), documentando la interpretación tomada donde el enunciado era
  ambiguo (ej. qué cuenta como "tickets abiertos").

El rol del humano en estas fases fue: dirigir el orden de las fases,
tomar las decisiones de alcance que Claude marcó como ambiguas
(ej. cómo reorganizar el repo, si usar Docker o Postgres nativo),
proporcionar el enunciado exacto de las 8 consultas SQL (no estaba en
los documentos de Fase 1), y ejecutar manualmente los pasos que
requerían privilegios de administrador que el entorno de Claude Code no
tenía (reiniciar el servicio de PostgreSQL, abrir Chrome).

**Estimado de la prueba apoyado en IA: ~95%.** El 5% restante son las
decisiones de producto/alcance y las acciones manuales de sistema
descritas arriba — todo el código, la arquitectura detallada y la
redacción de este documento fueron producidos por Claude.

## Estado del proyecto

Ver el checklist de fases en [`CLAUDE.md`](CLAUDE.md). Fases 1 a 6
completas.
